<section class="projects hover-underline">
    <div class="block">
      <div class="container">
        <div class="section-title sliding-u-l-r">Projects</div>
      </div>
        <div class="section-underline"></div>
        <div class="container">
          <a href="/Resume2019.docx" download="resume_han_khuu" class="btn btn-default btn-margin" role="button">Download Resume</a>
          <div class="row">
            <div class="col-12 col-lg-6">
              <div id="project-preview">
                <a href="https://bandbasher.com" target="_blank">
                  <img class="project-img" src="/img/projects/bandbasher_full.jpg" />
                  <div id="project-info" class="info-text">
                    <h4>bandbasher</h4>
                    <p>Helping artists/musicians develop their skills as well as branding themselves on social media. Providing users with lesson videos and quizzes, in addition to storing and sharing their music with like minded individuals.</p>
                    <p>Click to view more on bandbasher.com</p>
                  </div>
                </a>
              </div>
            </div>
            <div class="col-12 col-lg-6">
              <div id="project-preview2">
                <a href="https://github.com/hkhuu1011/recipes" target="_blank">
                  <img class="project-img" src="/img/projects/recipes_full.jpg" />
                  <div id="project-info2" class="info-text">
                    <h4>Recipes</h4>
                    <p>A recipe finder, using ingredients as keywords to find the recipe you're looking for! If you find something that you like, you can save it, as well as your own personal notes/modifications, and refer back to it later.</p>
                    <p>Click to view more on Github.</p>
                  </div>
                </a>
              </div>
            </div>
          </div>
        </div>
    </div>
    <div class="container">
      <small class="right" style="color:#ccc">(More projects in the works!)</small>
    </div>
  </section>